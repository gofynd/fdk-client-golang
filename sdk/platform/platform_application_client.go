...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppCatalog holds PlatformAppCatalog object properties
    ...
    type PlatformAppGetApplicationBrandsXQuery struct { 
        ...
        PageSize float64  `url:"page_size,omitempty"` 
        Q string  `url:"q,omitempty"` 
        Uids float64  `url:"uids,omitempty"`  
    }
    ...
    //PlatformAppGetLocationsXQuery holds query params
    type PlatformAppGetLocationsXQuery struct { 
    ...
    // GetLocations Get list of locations
     func (ca *PlatformAppCatalog)  GetLocations(xQuery PlatformAppGetLocationsXQuery) (LocationListSerializer, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
        ...
        return getLocationsResponse, nil
            ...
            func (ca *PlatformAppCatalog)  GetLocationsPaginator( xQuery PlatformAppGetLocationsXQuery ) *common.Paginator {
                paginator := common.NewPaginator("number")
                ...
                paginator.Next = func() (interface{}, error) {
                    response, err := ca.GetLocations(xQuery)
                    if response.Page.HasNext {
                        paginator.SetPaginator(response.Page.HasNext, int(response.Page.Current+1), response.Page.NextID)
                    }
                    return response, err
