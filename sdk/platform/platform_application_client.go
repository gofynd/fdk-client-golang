...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppCatalog holds PlatformAppCatalog object properties
    ...
    // UpdateAppLocation Update a single custom json.
     func (ca *PlatformAppCatalog)  UpdateAppLocation(BrandUID string, StoreUID string, body  ApplicationStoreJson) (SuccessResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            updateAppLocationResponse SuccessResponse
	    )
        ...
        rawRequest = NewRequest(
            ca.config,
            "patch",
            fmt.Sprintf("/service/platform/catalog/v1.0/company/%s/application/%s/store/%s",ca.CompanyID, ca.ApplicationID, BrandUID, StoreUID),
            nil,
            nil,
            reqBody)
        response, err = rawRequest.Execute()
