...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppPayment holds PlatformAppPayment object properties
    ...
    // GetPosPaymentModeRoutes Get COD limit for user
     func (pa *PlatformAppPayment)  GetPosPaymentModeRoutes() (GetUserCODLimitResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getPosPaymentModeRoutesResponse GetUserCODLimitResponse
	    )
        ...
        rawRequest = NewRequest(
            pa.config,
            "get",
            fmt.Sprintf("/service/platform/payment/v1.0/company/%s/application/%s/payment/fetch-cod-order-limit",pa.CompanyID, pa.ApplicationID),
            nil,
            nil,
            nil)
        response, err = rawRequest.Execute()
    ...
    // GetPosPaymentModeRoutes Set COD option for user for payment
     func (pa *PlatformAppPayment)  GetPosPaymentModeRoutes(body  SetCODForUserRequest) (SetCODOptionResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getPosPaymentModeRoutesResponse SetCODOptionResponse
	    )
