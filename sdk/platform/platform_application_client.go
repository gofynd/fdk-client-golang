...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppCatalog holds PlatformAppCatalog object properties
    ...
    // LocationReassignment Location Reassignment
     func (ca *PlatformAppCatalog)  LocationReassignment(body  AssignStore) (StoreAssignResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            locationReassignmentResponse StoreAssignResponse
	    )
        ...
        rawRequest = NewRequest(
            ca.config,
            "post",
            fmt.Sprintf("/service/platform/catalog/v1.0/company/%s/application/%s/location/reassign/",ca.CompanyID, ca.ApplicationID),
            nil,
            nil,
            reqBody)
        response, err = rawRequest.Execute()
        ...
        err = json.Unmarshal(response, &locationReassignmentResponse)
        if err != nil {
            return StoreAssignResponse{}, common.NewFDKError(err.Error())
        }
        return locationReassignmentResponse, nil
