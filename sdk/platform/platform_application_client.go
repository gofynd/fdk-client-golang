...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppCatalog holds PlatformAppCatalog object properties
    ...
    //PlatformAppGetAppInventoryStatusXQuery holds query params
    type PlatformAppGetAppInventoryStatusXQuery struct { 
        ItemID []float64  `url:"item_id,omitempty"` 
        StoreIds []float64  `url:"store_ids,omitempty"` 
        BrandIds []float64  `url:"brand_ids,omitempty"` 
        SellerIdentifier []float64  `url:"seller_identifier,omitempty"` 
        Timestamp string  `url:"timestamp,omitempty"` 
        PageSize float64  `url:"page_size,omitempty"` 
        PageID string  `url:"page_id,omitempty"`  
    }
    
    // GetAppInventoryStatus Get the stock of a product
     func (ca *PlatformAppCatalog)  GetAppInventoryStatus(xQuery PlatformAppGetAppInventoryStatusXQuery) (InventoryStockResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getAppInventoryStatusResponse InventoryStockResponse
	    )
        ...
        err = json.Unmarshal(response, &getAppInventoryStatusResponse)
        if err != nil {
            return InventoryStockResponse{}, common.NewFDKError(err.Error())
        }
        return getAppInventoryStatusResponse, nil
