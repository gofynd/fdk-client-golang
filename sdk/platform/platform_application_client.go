...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppPayment holds PlatformAppPayment object properties
    ...
    //PlatformAppEdcDeviceXQuery holds query params
    type PlatformAppEdcDeviceXQuery struct { 
        TerminalSerialNo string  `url:"terminal_serial_no,omitempty"`  
    }
    
    // EdcDevice get details of a single edc device
     func (pa *PlatformAppPayment)  EdcDevice(xQuery PlatformAppEdcDeviceXQuery) (EdcDeviceDetailsResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
    ...
    // EdcDevice map new edc device to the terminal
     func (pa *PlatformAppPayment)  EdcDevice(body  EdcAddRequest) (EdcDeviceAddResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
    ...
    // EdcDevice Update store id and device tag of edc device
     func (pa *PlatformAppPayment)  EdcDevice(body  EdcUpdateRequest) (EdcDeviceUpdateResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
    ...
    //PlatformAppGetPaymentModeRoutesPosXQuery holds query params
    type PlatformAppGetPaymentModeRoutesPosXQuery struct { 
        Refresh bool  `url:"refresh,omitempty"` 
        RequestType string  `url:"request_type,omitempty"`  
    }
    
    // GetPaymentModeRoutesPos Get All Valid Payment Options
     func (pa *PlatformAppPayment)  GetPaymentModeRoutesPos(xQuery PlatformAppGetPaymentModeRoutesPosXQuery) (PaymentOptionsResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getPaymentModeRoutesPosResponse PaymentOptionsResponse
	    )
        ...
        err = json.Unmarshal(response, &getPaymentModeRoutesPosResponse)
        if err != nil {
            return PaymentOptionsResponse{}, common.NewFDKError(err.Error())
        }
        return getPaymentModeRoutesPosResponse, nil
