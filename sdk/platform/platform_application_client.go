...
func NewApplicationClient(appID string, config *PlatformConfig) *ApplicationClient {
   ...
   // PlatformAppCatalog holds PlatformAppCatalog object properties
    ...
    // UpsertAppProductMeta Update a single custom meta.
     func (ca *PlatformAppCatalog)  UpsertAppProductMeta(ItemID string, body  ApplicationItemMeta) (SuccessResponse, error) {
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            upsertAppProductMetaResponse SuccessResponse
	    )
        ...
        err = json.Unmarshal(response, &upsertAppProductMetaResponse)
        if err != nil {
            return SuccessResponse{}, common.NewFDKError(err.Error())
        }
        return upsertAppProductMetaResponse, nil
