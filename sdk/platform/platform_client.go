...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformDocumentEngine holds PlatformDocumentEngine object properties
    ...
    //PlatformGetBulkShipmentStatusXQuery holds query params
    type PlatformGetBulkShipmentStatusXQuery struct { 
        BatchID string  `url:"batch_id,omitempty"`  
    }
    


    // GetBulkShipmentStatus Get Status of Bulk Invoice and Label generation
     func (do *PlatformDocumentEngine)  GetBulkShipmentStatus(xQuery PlatformGetBulkShipmentStatusXQuery) (SuccessResponseBulkStatus, error){
        
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getBulkShipmentStatusResponse SuccessResponseBulkStatus
	    )
        ...
        rawRequest = NewRequest(
            do.Config,
            "get",
            fmt.Sprintf("/service/platform/document/v2.0/company/%s/bulk-shipment-status",do.CompanyID),
            nil,
            xQuery,
            nil)
        response, err = rawRequest.Execute()
        if err != nil {
             return SuccessResponseBulkStatus{}, err
	    }
        
        err = json.Unmarshal(response, &getBulkShipmentStatusResponse)
        if err != nil {
             return SuccessResponseBulkStatus{}, common.NewFDKError(err.Error())
        }
        return getBulkShipmentStatusResponse, nil
