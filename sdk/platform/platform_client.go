...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformDocumentEngine holds PlatformDocumentEngine object properties
    ...
    // GeneratePOSReceipts Generate Receipts for an order
     func (do *PlatformDocumentEngine)  GeneratePOSReceipts(body  GeneratePOSReceipts) (SuccessResponseGeneratePOSReceipts, error){
        
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            generatePOSReceiptsResponse SuccessResponseGeneratePOSReceipts
	    )
        ...
        if err != nil {
             ...
             return SuccessResponseGeneratePOSReceipts{}, common.NewFDKError(err.Error())
        }
        err = json.Unmarshal([]byte(reqBodyJSON), &reqBody)
        if err != nil {
        ...
        rawRequest = NewRequest(
            do.Config,
            "post",
            fmt.Sprintf("/service/platform/document/v2.0/company/%s/pos/generate-receipt",do.CompanyID),
            nil,
            nil,
            reqBody)
        response, err = rawRequest.Execute()
        if err != nil {
             return SuccessResponseGeneratePOSReceipts{}, err
	    }
        
        err = json.Unmarshal(response, &generatePOSReceiptsResponse)
        if err != nil {
             return SuccessResponseGeneratePOSReceipts{}, common.NewFDKError(err.Error())
        }
        return generatePOSReceiptsResponse, nil
