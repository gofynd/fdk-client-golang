...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformBilling holds PlatformBilling object properties
    ...
    // CreateOneTimeSubscriptionCharge Create one time subscription charge
     func (bi *PlatformBilling)  CreateOneTimeSubscriptionCharge(ExtensionID string, body  CreateOneTimeSubscriptionCharge) (CreateSubscriptionResponse, error){
        
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            createOneTimeSubscriptionChargeResponse CreateSubscriptionResponse
	    )
        ...
        rawRequest = NewRequest(
            bi.Config,
            "post",
            fmt.Sprintf("/service/platform/billing/v1.0/company/%s/extension/%s/one_time",bi.CompanyID, ExtensionID),
            nil,
            nil,
            reqBody)
        response, err = rawRequest.Execute()
        ...
        err = json.Unmarshal(response, &createOneTimeSubscriptionChargeResponse)
        if err != nil {
             return CreateSubscriptionResponse{}, common.NewFDKError(err.Error())
        }
        return createOneTimeSubscriptionChargeResponse, nil
