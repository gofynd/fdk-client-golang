...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformCatalog holds PlatformCatalog object properties
    ...
    type PlatformGetProductAttributesXQuery struct { 
        Q string  `url:"q,omitempty"` 
        Department []interface{}  `url:"department,omitempty"` 
        Category string  `url:"category,omitempty"` 
        Filtered bool  `url:"filtered,omitempty"` 
        PageNo float64  `url:"page_no,omitempty"` 
        PageSize float64  `url:"page_size,omitempty"` 
        Sort float64  `url:"sort,omitempty"`  
    }
    


    // GetProductAttributes Get list of all the attributes
    ...
    // GetLaneConfig 
     func (or *PlatformOrders)  GetLaneConfig(xQuery PlatformGetLaneConfigXQuery) (LaneConfigResponse, error){
        ...
        rawRequest = NewRequest(
            or.Config,
            "get",
            fmt.Sprintf("/service/platform/orders/v1.0/company/%s/store/lane-config/",or.CompanyID),
            nil,
            xQuery,
            nil)
        response, err = rawRequest.Execute()
