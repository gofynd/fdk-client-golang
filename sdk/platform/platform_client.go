...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformOrders holds PlatformOrders object properties
    ...
    // GetPlatformShipmentReasons Get reasons behind full or partial cancellation of a shipment
     func (or *PlatformOrders)  GetPlatformShipmentReasons(ShipmentID string, BagID string, State string) (PlatformShipmentReasonsResponse, error){
        
        var (
            rawRequest  *RawRequest
            response    []byte
            err         error
            getPlatformShipmentReasonsResponse PlatformShipmentReasonsResponse
	    )
