...
func (p *PlatformClient) SetPlatformApplicationClient(appID string) {
   ...
   // PlatformPayment holds PlatformPayment object properties
    ...
    // PaymentStatusBulk Get Payment status and information for a list of order_ids
     func (pa *PlatformPayment)  PaymentStatusBulk(ApplicationID string, body  PaymentStatusBulkHandlerRequest) (PaymentStatusBulkHandlerResponse, error){
        ...
        rawRequest = NewRequest(
            pa.Config,
            "post",
            fmt.Sprintf("/service/platform/payment/v1.0/company/%s/payment-status-bulk/",pa.CompanyID, pa.ApplicationID),
            nil,
            nil,
            reqBody)
        response, err = rawRequest.Execute()
