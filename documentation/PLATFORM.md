...
data, err := Content.CreateInjectableTag(CompanyID, ApplicationID, body);
```
...
data, err := Catalog.UpdateCollection(CompanyID, ApplicationID, ID);
```
...
data, err := Catalog.ListDepartmentsData(CompanyID);
```
...
| xQuery | struct | Includes properties such as `Level`, `Q`
...
| xQuery | struct | Includes properties such as `BrandIds`, `CategoryIds`, `Search`, `PageNo`, `PageSize`
...
| ItemID | float64 | Item of the company associated to product that is to be viewed. | 


| Size | string | Size in which inventory is to be added. | 


| body |  InventoryRequest1 | "Request body" 
...
data, err := Inventory.UpdateJob(CompanyID, XUserData, body);
```
...
data, err := Inventory.CreateJob(CompanyID, XUserData, body);
```
